...
import utilClasses from './ds/utils/utility.module.css';

import { MultiSelectItem } from './MultiSelectItem';
import classes from './Select.module.css';
import { optionSearch } from './utils';
import type { SelectProps, SingleSelectEvent, MultiSelectEvent, MultiSelectOption } from './types';
import { OptionList } from './OptionList';
import { usePrevious } from 'app-shared/hooks/usePrevious';
import { useUpdate } from 'app-shared/hooks/useUpdate';
import { arraysEqual, objectValuesEqual } from 'app-shared/components/Select/ds/utils';
import { useFocusWithin } from 'app-shared/components/Select/ds/hooks/useFocusWithin';
import { useKeyboardEventListener } from 'app-shared/components/Select/ds/hooks';
import { InputWrapper } from 'app-shared/components/Select/ds/_InputWrapper';
...
const Select = (props: SelectProps) => {
  ...
  const [keyword, setKeyword] = useState(!multiple ? findOptionFromValue(value)?.label ?? '' : '');
  ...
  useUpdate(() => {
    ...
    if (
      ...
      options.some((option, index) => !objectValuesEqual(option, prevOptions[index]))
    ) {
    ...
    if (
      ...
      (multiple && (typeof prevValue === 'string' || !arraysEqual(value, prevValue))) ||
      shouldSetValue
    ) {
      if (multiple) {
  ...
  const activeOptionIndex = sortedOptions.findIndex((option) => option.value === activeOption);
  ...
  useUpdate(() => {
    ...
    if (hasFocus && onFocus) multiple ? onFocus(selectedValues) : onFocus(activeOption || '');
    else if (!hasFocus && onBlur) multiple ? onBlur(selectedValues) : onBlur(activeOption || '');
  }, [hasFocus]);
        ...
        inputRenderer={({ className, inputId: id, hasIcon }) => (
          ...
          >
            <span className={classes.inputArea}>
              {multiple && (
                <>
                  {selectedValues.map(findOptionFromValue).map((o) => (
                    <MultiSelectItem
                      deleteButtonLabel={(o as MultiSelectOption).deleteButtonLabel}
              ...
              <input
                aria-activedescendant={activeOption ? `${id}-${activeOption}` : undefined}
