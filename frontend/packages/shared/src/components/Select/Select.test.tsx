...
jest.mock('./utils', () => ({
  optionSearch: (options: SingleSelectOption[] | MultiSelectOption[], keyword: string) =>
...
describe('Select', () => {
  ...
  describe('Single select', () => {
    ...
    it('Calls onFocus handler with the selected value only when the search field is focused', async () => {
      ...
      render(
        <>
          <Select {...defaultSingleSelectProps} {...{ onFocus, value }} />
    ...
    it('Calls onBlur handler with the selected value when focus switches to another element', async () => {
      ...
      render(
        <>
          <Select {...defaultSingleSelectProps} {...{ onBlur, value }} />
          <button data-testid={outsideButtonTestid}>Test</button>
        </>,
      );
    ...
    it('Rerenders with new options when the "options" property changes', async () => {
      ...
      rerender(<Select {...defaultSingleSelectProps} options={newOptions} />);
      await act(() => user.click(screen.getByRole('combobox')));
      await act(() => user.click(screen.getAllByRole('option')[1]));
    ...
    it('Rerenders with the correct value and keyword when "options" are loaded late', () => {
      ...
      rerender(<Select {...defaultSingleSelectProps} options={options} value={value} />);
  ...
  describe('Multiple select', () => {
    ...
    it('Selects options on enter key press', async () => {
      ...
      expectSelectedValues([multiSelectOptions[0].value, multiSelectOptions[2].value]);
    });
    ...
    it('Rerenders with new options when the "options" property changes', async () => {
      ...
      const newOptions: Required<Omit<MultiSelectOption, 'keywords' | 'formattedLabel'>>[] = [
    ...
    it('Calls onFocus handler with the selected values only when the search field is focused', async () => {
      ...
      render(
        <>
          <Select {...defaultMultiSelectProps} {...{ onFocus, value }} />
          <button data-testid={outsideButtonTestid}>Test</button>
        </>,
      );
      expect(onFocus).not.toHaveBeenCalled();
    ...
    it('Calls onBlur handler with the selected values when focus switches to another element', async () => {
      ...
      render(
        <>
          <Select {...defaultMultiSelectProps} {...{ onBlur, value }} />
          <button data-testid={outsideButtonTestid}>Test</button>
        </>,
      );
      expect(onBlur).not.toHaveBeenCalled();
    ...
    const getFocusedOption = () => screen.getByRole('listbox').querySelector('[class*="focused"]');
    ...
    const expectSelectedValues = (values: string[]) =>
      getOptions().forEach((opt) => {
        ...
        expect(opt).toHaveAttribute('aria-selected', `${isSelected.toString()}`);
      });
  });
});

const renderSingleSelect = (props?: Partial<SingleSelectProps>) =>
  render(<Select {...defaultSingleSelectProps} {...props} />);

const renderMultiSelect = (props?: Partial<MultiSelectProps>) =>
  render(<Select {...defaultMultiSelectProps} {...props} />);
...
const expectSelectedOptions = (predicate: (index: number) => boolean) =>
  getOptions().forEach((option, index) => {
    predicate(index) ? expectItemToBeSelected(option) : expectItemNotToBeSelected(option);
  });
