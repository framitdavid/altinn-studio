...
import { ErrorMessage, Paragraph } from '@digdir/design-system-react';
...
export type CharacterLimit = Omit<CharacterCounterProps, 'ariaDescribedById' | 'value'>;
...
export const InputWrapper = ({
  ...
  const characterLimitDescriptionId = characterLimit ? autoCharLimitIdGenerated : undefined;
  ...
  return (
    <span id={id} className={className}>
      <span className={cn(classes.inputAndLabel, hasIcon && classes.withIcon)}>
        {label && (
          <label className={classes.label} htmlFor={givenOrRandomInputId}>
            {label}
          </label>
        )}
        <span
        ...
        >
          ...
          {inputRenderer({
            className: cn(classes.field, !noFocusEffect && utilityClasses.focusable),
            hasIcon,
            inputId: givenOrRandomInputId,
            variant,
            describedBy: buildAriaDescribedBy([ariaDescribedBy, characterLimitDescriptionId]),
          })}
        </span>
      </span>
...
const CharacterCounter = ({
  ...
  return (
    <>
      <span className={utilityClasses.visuallyHidden} id={ariaDescribedById}>
        {srLabel}
      </span>
      <span className={classes.characterLimitLabel}>
        {hasExceededLimit ? (
          <ErrorMessage as='span' aria-live='polite' size='small'>
            {label(currentCount)}
          </ErrorMessage>
        ) : (
